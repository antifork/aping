...
/*----------------------------------------------------------------------------*/
void *
b_calloc(size_t items, size_t size)
{
	void *a;
	a = calloc(items, size);
	if (a)
...
/*----------------------------------------------------------------------------*/
LAYER *
alloc_layer(u_32 size, u_32 psize, u_32 nsize)
{
	LAYER *l_temp;
	NEURO *n_temp;
	NEURO *oldn_temp = 0;
	...
	for (csize = 0; csize != size; csize++) {
		...
		if (nsize)	/* if there is a next_layer we alloc forward
				 * prop links */
...
/*----------------------------------------------------------------------------*/
void 
link_layers(LAYER * layer_a, LAYER * layer_b)
{
	NEURO *neuro_a;
	NEURO *neuro_b;
	LINK *link;
...
load_nn(char *filename)
{
	...
	for (counter = 0, counter2 = 0; counter2 != pivot16; counter2++)
		counter += p_in_layers[counter2];
...
void 
fire_nn(NN * nn, double *input, double *expected, double *output)
{
	LAYER *c_layer;
	NEURO *c_neuro;
	LINK *c_link;
		...
		/* for_each(layer) */
	{
			...
			/* for_each(neuron) */
		{
				...
				/* for_each(link) */
			{
				...
				c_link->linkedneuro->state += c_link->weight * c_neuro->output;	/* output of lower
												 * neuron */
			}
			NN_VERBOSE("\r\n");
			/*
			 * since we have computed output for c_layer in a
			 * previus for_each(layer) cicle we can calculate
			 * output and zero all neurons->state
			 */
...
int 
feed_from_file(NN * nn, char *filename, FEEDCALLBACK * feedcallback)
{
	FILE *fp;
	double *input;
	double *output;
	...
	for (l_tmp = nn->first, counter = 0; counter != nn->size - 1; counter++)
		l_tmp = l_tmp->next;
