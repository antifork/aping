...
{
	/* destination, redirect_type , tos, new_gateway, icmp_src_ip, icmp_dest_ip */
	checkargs(argv,6,ARG_IP,ARG_NUM,ARG_NUM,ARG_IP,ARG_IP,ARG_IP,ARG_IP);

	ICMP_type(p)= ICMP_REDIRECT;
	ICMP_code(p)= ((int)strtol(argv[1], (char **)NULL, 10))%4;

	ICMP_gwaddr(p)= gethostbyname_lru(argv[3]);
	...
	ICMP_IP_src(p)  = gethostbyname_lru(argv[4]);
	ICMP_IP_dst(p)  = gethostbyname_lru(argv[5]);
